...
type, public :: MOM_control_struct ; private
  ...
  logical :: USE_LA_LI2016           !< If true, LF17 wavemethod update Stokes drift
...
subroutine step_MOM(forces_in, fluxes_in, sfc_state, Time_start, time_int_in, CS, &
  ...
  real,  dimension(SZI_(CS%G),SZJ_(CS%G))  :: nx_wind, ny_wind
  ...
  if (do_dyn) then
    ...
    if (CS%UseWaves) then
      ...
      call find_ustar(forces, CS%tv, U_star, G, GV, US, halo=1,nx_wind=nx_wind,ny_wind=ny_wind)
      call thickness_to_dz(h, CS%tv, dz, G, GV, US, halo_size=1)
      call Update_Stokes_Drift(G, GV, US, Waves, dz, U_star, time_interval, do_dyn,nx_wind=nx_wind,ny_wind=ny_wind)
      call disable_averaging(CS%diag)
    endif
  else ! not do_dyn.
    if (CS%UseWaves) then ! Diagnostics are not enabled in this call.
      call find_ustar(fluxes, CS%tv, U_star, G, GV, US, halo=1,nx_wind=nx_wind,ny_wind=ny_wind)
      call thickness_to_dz(h, CS%tv, dz, G, GV, US, halo_size=1)
      call Update_Stokes_Drift(G, GV, US, Waves, dz, U_star, time_interval, do_dyn,nx_wind=nx_wind,ny_wind=ny_wind)
    endif
  endif
...
# include "version_variable.h"
  ...
  call get_param(param_file, "MOM", "USE_WAVES", CS%UseWaves, default=.false., &
                 do_not_log=.true.)
  call get_param(param_file, "MOM", "USE_LA_LI2016", CS%USE_LA_LI2016, default=.false., &
                 do_not_log=.true.)
  CS%UseWaves = CS%UseWaves .or. CS%USE_LA_LI2016 !so we update stokes drift when we have/not wave information
