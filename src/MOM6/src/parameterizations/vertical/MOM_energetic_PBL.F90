...
type, public :: energetic_PBL_CS ; private
  ...
  real :: Shear_ePBL_LT_cap = 10 !the cap for mstar_shear/mstar_ePBL, which means enhance factor should be smaller than this
  real :: LT_shear_frac_cap = 1.2! the fraction ratios that LT rescale cannot exceed to the mstar_shear if shear eneryg coupling
  real :: Ustar_min_thre=1.0e-4  !the minimum ustar value for shear energy coupling to happen 
  !/ vertical decay related options  
...
! interface.
  ...
  do OBL_it=1,CS%Max_MLD_Its
    ...
    if ((.not. CS%use_shear_energy_coupling) .or. (u_star <= CS%Ustar_min_thre)) then        
        if (CS%Use_LT) then
        ...
        eCD%used_shear_flag = 0.0        
         ...
         if (present(use_shear_energy) .and. use_shear_energy .and. &
             ...
             eCD%mstar_shear  = mstar_shear
               ...
               hmin = 1.0 * dz(1)           ! 也可以用 3.0*dz(1) 更保守
               thick_ok = (abs(Hbl_est) >= hmin)
             if ((E_shear > E_ePBL_local) .and. mstar_shear_ok .and. thick_ok) then
         ...
         if (CS%Use_LT) then
          ...
          ! --- reference LT on ePBL base to form fraction  (no shear rescale) ---
             ...
             if (CS%LT_rescale_by_shear) then
                ...
                (mstar_LT0 < 0.8 * mstar_total) .and. (max(mstar_total,mstar_e_local)> 1.0e-5)  )  then
                 ...
                 case ('preserve_fraction')     !keep the mstar_LT/(mstar_ePBL) ratio the same
                     scale_need= 1.0
                     if ((used_shear_flag>0.5) .and. (min(mstar_total,mstar_e_local)> 1.0e-5)) then
                           base0 = mstar_total / max(mstar_e_local, 1.0e-5)
                           scale_need = min(max(base0**CS%LT_rescale_gamma,1.0),CS%Shear_ePBL_LT_cap)
                           scale_need=max(min(scale_need, CS%LT_shear_frac_cap*mstar_total/max(mstar_LT0,1.0e-5)),1.0)
  ...
  call get_param(param_file, mdl, "SHEAR_EPBL_CAP", CS%Shear_ePBL_cap, &
       ...
       "normaly would be smaller than 50.0", &
               units="nondim", default=30.0,&
                 do_not_log=(CS%mstar_scheme==Use_Fixed_MStar .and. (.not. CS%use_shear_energy_coupling)))
  call get_param(param_file, mdl, "SHEAR_EPBL_LT_CAP", CS%Shear_ePBL_LT_cap, &
               "when use shear energy ePBL coupling, the cap ratio for LT enhancement factor"\
       "normaly would be smaller than 50.0", &
               units="nondim", default=10.0,&
                 do_not_log=(CS%mstar_scheme==Use_Fixed_MStar .and. (.not. CS%use_shear_energy_coupling)))
  call get_param(param_file, mdl, "LT_SHEAR_FRAC_CAP", CS%LT_shear_frac_cap, &
               "when use shear energy ePBL coupling, the cap fraction ratio for LT enhancement factor to be smaller than mstar_shear"\
       "normaly would be smaller than 2.0", &
               units="nondim", default=1.2,&
                 do_not_log=(CS%mstar_scheme==Use_Fixed_MStar .and. (.not. CS%use_shear_energy_coupling)))    
  call get_param(param_file, mdl, "USTAR_MIN_THRE", CS%Ustar_min_thre, &
               "when use shear energy ePBL coupling, the threshold value to stop shear energy replacement"\
       "normaly would be smaller than 0.01", &
